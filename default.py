...
def listLinks(section, url, showtitle='', showyear=''): ### Menu for Listing Hosters (Host Sites of the actual Videos)
	...
	if  ( section == 'tv'): ## TV Show ## Title (Year) - Info
		...
		#match=re.compile(s, re.MULTILINE | re.IGNORECASE | re.DOTALL).findall(html)[0]
		#if (_debugging==True): print match
		#if (match==None): return
		##ShowYear=match[1].strip(); ShowTitle=match[0].strip(); ShowTitle=HTMLParser.HTMLParser().unescape(ShowTitle); ShowTitle=ParseDescription(ShowTitle); 
		##ShowTitle=ShowTitle.encode('ascii', 'ignore'); ShowTitle=ShowTitle.decode('iso-8859-1'); 
		##ShowPlot=(re.compile(ps('LLinks.compile.show.plot'), re.MULTILINE | re.IGNORECASE | re.DOTALL).findall(html)[0]).strip(); ShowPlot=HTMLParser.HTMLParser().unescape(ShowPlot); ShowPlot=ParseDescription(ShowPlot); ShowPlot=ShowPlot.encode('ascii', 'ignore'); ShowPlot=ShowPlot.decode('iso-8859-1'); match=re.compile(ps('LLinks.compile.imdb.url_id'), re.MULTILINE | re.IGNORECASE | re.DOTALL).findall(html)[0]
		#if (_debugging==True): print match
		#(IMDbURL,IMDbID)=match; IMDbURL=IMDbURL.strip(); IMDbID=IMDbID.strip(); My_infoLabels={ "Studio": ShowTitle+'  ('+ShowYear+')', "Title": ShowTitle, "ShowTitle": ShowTitle, "Year": ShowYear, "Plot": ShowPlot, 'IMDbURL': IMDbURL, 'IMDbID': IMDbID, 'IMDb': IMDbID }; listitem.setInfo(type="Video", infoLabels=My_infoLabels )
	### Both -Movies- and -TV Shows- ### Hosters
	...
	if (len(match) > 0):
		...
		#for url, host, quality, age in match:
		print match
		for url, blah, host in match:
			host=host.strip() #; quality=quality.strip(); 
			host2=re.compile('http://([0-9A-Za-z\-\.]+)/').findall(url)[0]
			name=str(count)+". "+host #+' - [[B]'+quality+'[/B]] - ([I]'+age+'[/I])'
			if urlresolver.HostedMediaFile(host=host2, media_id='xxx'):
				print (url, blah, host)
				#img=ps('Hosters.icon.url')+host; 
				img=_param['img']
				#My_infoLabels['quality']=quality; My_infoLabels['age']=age; My_infoLabels['host']=host
				...
				pars={'section': section, 'img': _param['img'], 'mode': 'PlayVideo', 'url': url}
				contextMenuItems=[]; 
...
def Menu_BrowseByGenre(section=_default_section_):
	...
	for genre in GENRES:
		...
		if (img=='') and (browsebyImg is not ''): img=browsebyImg
		if (img==''): img=_artSun
		if section == ps('section.movie'): 	url=_domain_url+'/category/'+(genre.lower().replace(' ','-'))+'/'
		#else: 															url=_domain_url+ps('BrowseByGenre.tv.url1')		+(genre.lower().replace(' ','-'))+ps('BrowseByGenre.tv.url2')
	...
	#GENREStv=ps('GENREStv')
	#ItemCount=len(GENREStv) # , total_items=ItemCount
	...
	#	if (img=='') and (browsebyImg is not ''): img=browsebyImg
	#	if (img==''): img=_artSun
	#	if section == ps('section.tv'): 	url=_domain_url+'/category/'+(genre.lower().replace(' ','-'))+'/'
	#	#else: 															url=_domain_url+ps('BrowseByGenre.tv.url1')		+(genre.lower().replace(' ','-'))+ps('BrowseByGenre.tv.url2')
	#	_addon.add_directory({'section': section,'mode': 'GetTitles','url': url,'genre': genre,'bygenre': genre,'pageno': '1','pagecount': addst('pages')}, {'title':  genre},img=img,fanart=_artFanart, total_items=ItemCount)
	set_view('list',addst('default-view')); eod()
...
def listItems(section=_default_section_, url='', startPage='1', numOfPages='1', genre='', year='', stitle='', season='', episode='', html='', chck=''): # List: Movies or TV Shows
	...
	urlSplitter='/page/'; deb('urlSplitter',urlSplitter)
	...
	#	set_view('episodes',515); _addon.end_of_directory(); return
		...
		##s+='<p>\s*<span\sclass="\D+">Movie Name:</span>\s*<a\shref="http://vidics4.com/watch-[0-9A-Za-z\-]+/"\stitle="Watch\s.+?">Watch (.+?) \((\d\d\d\d)\) Online Movie</a></p>[\n]\s*'
		##s+='<p><span\sclass="\D+">Genre:</span>\s*(.+?)</p>[\n]\s*[\n]\s*[\n]\s*[\n]\s*'
		##s+='<p><span class="\D+">Release Date:</span> (\D+ \d+, \d\d\d\d)</p>[\n]\s*'
		##s+='<p><span class="\D+">Director:</span>\s(.+?)</p>[\n]\s*'
		##s+='<p><span class="\D+">Cast:</span>\s(.+?)</p>[\n]\s*[\n]\s*[\n]\s*[\n]\s*[\n]\s*'
		...
		#try:			iitems=re.compile(s, re.MULTILINE | re.IGNORECASE | re.DOTALL).findall(html)
		#except:		iitems=None
		...
		for url,img,title,year in iitems:
			...
			#	except: 
			#		uname=name; name='[Unknown]'; _addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': _domain_url + item_url, 'img': thumbnail, 'title': name, 'year': year }, {'title':  name+'  ('+year+')'}, img=thumbnail, contextmenu_items=contextMenuItems, total_items=ItemCount)
...
def Menu_LoadCategories(section=_default_section_): #Categories
	...
	#el
	if  ( section == ps('section.movie')): ##### Movies ###################################################################################################################################################################################
		# 
		# Cinema Movies
		# Featured
		# HD Movies
		# Latest Added
		# http://viooz.co/rss.xml
		# http://viooz.co/year/
		# http://viooz.co/host/
		# http://viooz.co/language/
		# http://viooz.co/country/
		# http://viooz.co/hd/
		# Featured http://viooz.co/top/
		# http://viooz.co/cenima/
		# http://viooz.co/genre/
		# http://viooz.co/movies/
		# http://viooz.co/title/0-9
		# http://viooz.co/title/A
		# Search (Title|Actor|Director)
		# 
		_addon.add_directory({'section': section, 'mode': 'Search', 				'pageno': '1', 'pagecount': addst('pages')},			{'title':  cFL('S',ps('cFL_color'))+'earch'}, 						fanart=_artFanart,img=art('icon-search'))
		_addon.add_directory({'section': section, 'mode': 'AdvancedSearch', 'pageno': '1', 'pagecount': addst('pages')},	 		{'title':  cFL('A',ps('cFL_color'))+'dvanced Search'}, 		fanart=_artFanart,img=art('icon-search'))
